...
| `getMediaItemsByCategory(category, locale?)` | `ImageWithAlt[]` | Dispatches by Media-page category id (`events`, `projects`, `hero`, `what-is-bears`, `about-us`, `page-banners`). `events`/`projects` return covers + inline `<Img>` blocks parsed from each post body (filtered by `displayInMedia`). `about-us` aggregates the about-us page hero + the "Our Mission" section image. `page-banners` aggregates the events/projects/sponsors/contact title heroes plus the media-page hero. All entries respect their per-image `displayInMedia` toggle. Each branch projects into `ImageWithAlt` (image + alt + optional description, where `description` is a single field used for caption + photographer credit). The `people` and `all` categories are handled directly in `media.astro`. Returns `[]` for unknown categories. |
