...
|------|------|---------|-------------|
| `images` | `ImageWithAlt[]` | &mdash; | **Required.** Array of `{ image, alt, description? }` entries; `image` is an `ImageMetadata` import. When `description` is set, a gradient strip renders on the bottom of that image (and beneath it in the click-to-enlarge modal). Use `description` for any combination of caption and photographer credit — there's no separate copyright field. |
