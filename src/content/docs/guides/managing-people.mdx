...
| `coverImageDescription` | string (optional) | Caption shown over the portrait on the Media page. Use it for any combination of caption and photographer credit (e.g. `"Akshat at WS2025 — © 2025 Jane Doe"`). Doesn't appear on Faces of BEARS / Meet the Team / Testimonials cards (those use custom layouts). |
| `showInFaces` | boolean | Single consent toggle for both the Faces of BEARS grid (About Us page) and the People accordion on the Media page. Default `false` (opt-in) — flip on per person once they've agreed to public display. |
| `order` | number | Sort order. Lower numbers appear first wherever this person is shown (Faces of BEARS grid, Media page). Ties break on the slug. |
