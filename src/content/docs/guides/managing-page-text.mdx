...
Each `.mdx` file provides text for a specific section of a page. You only edit the **frontmatter** (the YAML between the `---` lines) &mdash; the body of these files is not used.
...
**Finding the coordinates:** Open [google.com/maps](https://www.google.com/maps), right-click the spot you want to pin, and the first two numbers in the popup are the latitude and longitude — copy them straight into `mapLat` and `mapLng`.
