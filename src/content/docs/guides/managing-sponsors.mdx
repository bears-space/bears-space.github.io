...
| `logo` | string | Path to the logo image under `src/assets/sponsors/<tier>/<slug>/`, written as `/<slug>/<filename>` |
...
| `alt` | string | falls back to `name` | Alt text used by screen readers on the sponsor logo. Override only if the sponsor name isn't a good description. |
